# Implementation Plan: FEAT03 - Gestión de Usuarios (Iteration 2) - FINAL
...
- **Naming Convention:** 
    ...
    - **Identifiers:** `snake_case` for all column and table identifiers.
...
- Implement NestJS controllers and services for all entities using existing patterns.
- **Language Management:** Endpoints for `tb_languages` (seeded with 'en' and 'es').
- **JSONB Handling:** Manage structured translation objects in `name` and `description` (e.g., `{"en": "Text", "es": "Texto"}`).
- **Role Sync:** Sync `allowed_roles` with Keycloak role names.

### Security (RBAC)
- Implement `RolesGuard` in the BFF.
- **Admin Only:** Only users with the **'Admin'** role in Keycloak can access Iteration 2 management endpoints.
...
- **Interfaces:**
    - Use `Record<string, string>` for localized fields (`name`, `description`).
...
- Create a **Reusable Translation Table Modal**:
    - Reusable for `name` and `description` attributes.
    - First column: Language name (fetched from `tb_languages`).
    - Second column: Inline editable value for each translation.

### Role Selector Component
- Reusable checkbox list/combobox to select Keycloak roles for **Features** and **Actions**.

### Management Pages
- Based on the "User Directory" layout in the `security` microui.
- Integration of the Translation Editor popup and Role Selector.
- **Toasts:** Real-time feedback via `ToastService` for all successful or failed operations.
...
- **Workflow Test:** 
    ...
    2. Create a Feature: set translations for name/description and assign specific roles.
    3. Verify data persists correctly in PostgreSQL JSONB fields.
